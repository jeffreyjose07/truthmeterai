...
async function getCollectors(context: vscode.ExtensionContext) {
    if (!aiCollector) {
        ...
        aiCollector = new AIEventCollector(stor, context);
        codeCollector = new CodeChangeCollector(stor, context);
        timeTracker = new TimeTracker(stor, context);
